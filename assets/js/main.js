...
function checkAccess() {
  ...
  if (sidebarNav) {
    ...
    sidebarNav.querySelectorAll('a').forEach(link => { if (link.getAttribute('href') === page) link.closest('li')?.classList.add('active'); });
  }
...
async function loadActiveTickets() {
  try {
    ...
    const inc=data.incidents||[];
    const badge=document.querySelector('.badge-count'); if(badge) badge.textContent=`${inc.length} active`;
    const tbody=document.querySelector('.tickets-table tbody'); if(!tbody) return;
...
function closeEscalateModal() { document.getElementById('escalateModal').close(); document.getElementById('escalateForm')?.reset(); document.getElementById('escalateErrorMessage').textContent=''; }
async function handleEscalateIncident(event) {
  ...
  if (!dept)   { errEl.textContent='Please select a department.'; return; }
  if (!notes||notes.length<15) { errEl.textContent='Please provide escalation notes (min 15 chars).'; return; }
  btn.textContent='Escalating...'; btn.disabled=true;
  try {
    await apiFetch(`/incidents/${currentEscalateTicket}/escalate`, { method:'PATCH', body:JSON.stringify({ escalationReason:reason, escalateTo:dept, escalationNotes:notes, priorityUpdate:priUpd }) });
    closeEscalateModal();
